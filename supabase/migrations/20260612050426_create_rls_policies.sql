...
DROP POLICY IF EXISTS "profiles_select" ON profiles; CREATE POLICY "profiles_select" ON profiles FOR SELECT
  TO authenticated
...
DROP POLICY IF EXISTS "profiles_update" ON profiles; CREATE POLICY "profiles_update" ON profiles FOR UPDATE
  TO authenticated
  USING (can_access_user(auth.uid(), id))
  WITH CHECK (can_access_user(auth.uid(), id));

DROP POLICY IF EXISTS "profiles_delete" ON profiles;
DROP POLICY IF EXISTS "profiles_delete" ON profiles; CREATE POLICY "profiles_delete" ON profiles FOR DELETE
  TO authenticated
  USING (EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role = 'super_admin'));

-- RLS Policies for clients
DROP POLICY IF EXISTS "clients_select" ON clients;
DROP POLICY IF EXISTS "clients_select" ON clients; CREATE POLICY "clients_select" ON clients FOR SELECT
  TO authenticated
...
DROP POLICY IF EXISTS "clients_delete" ON clients; CREATE POLICY "clients_delete" ON clients FOR DELETE
  TO authenticated
  USING (can_access_user(auth.uid(), agent_id));

-- RLS Policies for policies
DROP POLICY IF EXISTS "policies_select" ON policies;
DROP POLICY IF EXISTS "policies_select" ON policies; CREATE POLICY "policies_select" ON policies FOR SELECT
  TO authenticated
  USING (can_access_user(auth.uid(), agent_id));

DROP POLICY IF EXISTS "policies_insert" ON policies;
DROP POLICY IF EXISTS "policies_insert" ON policies; CREATE POLICY "policies_insert" ON policies FOR INSERT
  TO authenticated
  WITH CHECK (can_access_user(auth.uid(), agent_id));

DROP POLICY IF EXISTS "policies_update" ON policies;
DROP POLICY IF EXISTS "policies_update" ON policies; CREATE POLICY "policies_update" ON policies FOR UPDATE
  TO authenticated
  USING (can_access_user(auth.uid(), agent_id))
  WITH CHECK (can_access_user(auth.uid(), agent_id));

DROP POLICY IF EXISTS "policies_delete" ON policies;
DROP POLICY IF EXISTS "policies_delete" ON policies; CREATE POLICY "policies_delete" ON policies FOR DELETE
  TO authenticated
  USING (can_access_user(auth.uid(), agent_id));

-- RLS Policies for installments
DROP POLICY IF EXISTS "installments_select" ON installments;
DROP POLICY IF EXISTS "installments_select" ON installments; CREATE POLICY "installments_select" ON installments FOR SELECT
  TO authenticated
...
DROP POLICY IF EXISTS "installments_insert" ON installments; CREATE POLICY "installments_insert" ON installments FOR INSERT
  TO authenticated
...
DROP POLICY IF EXISTS "installments_update" ON installments; CREATE POLICY "installments_update" ON installments FOR UPDATE
  TO authenticated
  USING (EXISTS (SELECT 1 FROM policies WHERE policies.id = policy_id AND can_access_user(auth.uid(), policies.agent_id)))
  WITH CHECK (EXISTS (SELECT 1 FROM policies WHERE policies.id = policy_id AND can_access_user(auth.uid(), policies.agent_id)));

DROP POLICY IF EXISTS "installments_delete" ON installments;
DROP POLICY IF EXISTS "installments_delete" ON installments; CREATE POLICY "installments_delete" ON installments FOR DELETE
  TO authenticated
  USING (EXISTS (SELECT 1 FROM policies WHERE policies.id = policy_id AND can_access_user(auth.uid(), policies.agent_id)));
...
DROP POLICY IF EXISTS "targets_select" ON targets; CREATE POLICY "targets_select" ON targets FOR SELECT
  TO authenticated
...
DROP POLICY IF EXISTS "targets_update" ON targets; CREATE POLICY "targets_update" ON targets FOR UPDATE
  TO authenticated
  USING (can_access_user(auth.uid(), user_id))
...
DROP POLICY IF EXISTS "tasks_select" ON tasks; CREATE POLICY "tasks_select" ON tasks FOR SELECT
  TO authenticated
  USING (assigned_to = auth.uid() OR can_access_user(auth.uid(), assigned_to));

DROP POLICY IF EXISTS "tasks_insert" ON tasks;
DROP POLICY IF EXISTS "tasks_insert" ON tasks; CREATE POLICY "tasks_insert" ON tasks FOR INSERT
  TO authenticated
  WITH CHECK (true);

DROP POLICY IF EXISTS "tasks_update" ON tasks;
DROP POLICY IF EXISTS "tasks_update" ON tasks; CREATE POLICY "tasks_update" ON tasks FOR UPDATE
  TO authenticated
  USING (assigned_to = auth.uid() OR can_access_user(auth.uid(), assigned_to))
  WITH CHECK (assigned_to = auth.uid() OR can_access_user(auth.uid(), assigned_to));

DROP POLICY IF EXISTS "tasks_delete" ON tasks;
DROP POLICY IF EXISTS "tasks_delete" ON tasks; CREATE POLICY "tasks_delete" ON tasks FOR DELETE
  TO authenticated
  USING (EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role IN ('super_admin', 'dev_manager')));

-- RLS Policies for notifications
DROP POLICY IF EXISTS "notifications_select" ON notifications;
DROP POLICY IF EXISTS "notifications_select" ON notifications; CREATE POLICY "notifications_select" ON notifications FOR SELECT
  TO authenticated
  USING (user_id = auth.uid());

DROP POLICY IF EXISTS "notifications_insert" ON notifications;
DROP POLICY IF EXISTS "notifications_insert" ON notifications; CREATE POLICY "notifications_insert" ON notifications FOR INSERT
  TO authenticated
  WITH CHECK (true);

DROP POLICY IF EXISTS "notifications_update" ON notifications;
DROP POLICY IF EXISTS "notifications_update" ON notifications; CREATE POLICY "notifications_update" ON notifications FOR UPDATE
  TO authenticated
  USING (user_id = auth.uid())
  WITH CHECK (user_id = auth.uid());

DROP POLICY IF EXISTS "notifications_delete" ON notifications;
DROP POLICY IF EXISTS "notifications_delete" ON notifications; CREATE POLICY "notifications_delete" ON notifications FOR DELETE
  TO authenticated
  USING (user_id = auth.uid());

-- RLS Policies for month_closings
DROP POLICY IF EXISTS "month_closings_select" ON month_closings;
DROP POLICY IF EXISTS "month_closings_select" ON month_closings; CREATE POLICY "month_closings_select" ON month_closings FOR SELECT
  TO authenticated
  USING (true);
...
DROP POLICY IF EXISTS "month_closings_delete" ON month_closings; CREATE POLICY "month_closings_delete" ON month_closings FOR DELETE
  TO authenticated
  USING (EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role = 'super_admin'));
...
DROP POLICY IF EXISTS "audit_logs_insert" ON audit_logs; CREATE POLICY "audit_logs_insert" ON audit_logs FOR INSERT
  TO authenticated
  WITH CHECK (user_id = auth.uid());

DROP POLICY IF EXISTS "audit_logs_update" ON audit_logs;
DROP POLICY IF EXISTS "audit_logs_update" ON audit_logs; CREATE POLICY "audit_logs_update" ON audit_logs FOR UPDATE
  TO authenticated
  USING (false)
  WITH CHECK (false);

DROP POLICY IF EXISTS "audit_logs_delete" ON audit_logs;
DROP POLICY IF EXISTS "audit_logs_delete" ON audit_logs; CREATE POLICY "audit_logs_delete" ON audit_logs FOR DELETE
  TO authenticated
  USING (false);

-- RLS Policies for system_settings
DROP POLICY IF EXISTS "system_settings_select" ON system_settings;
DROP POLICY IF EXISTS "system_settings_select" ON system_settings; CREATE POLICY "system_settings_select" ON system_settings FOR SELECT
  TO authenticated
  USING (true);

DROP POLICY IF EXISTS "system_settings_insert" ON system_settings;
DROP POLICY IF EXISTS "system_settings_insert" ON system_settings; CREATE POLICY "system_settings_insert" ON system_settings FOR INSERT
  TO authenticated
  WITH CHECK (EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role = 'super_admin'));

DROP POLICY IF EXISTS "system_settings_update" ON system_settings;
DROP POLICY IF EXISTS "system_settings_update" ON system_settings; CREATE POLICY "system_settings_update" ON system_settings FOR UPDATE
  TO authenticated
  USING (EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role = 'super_admin'))
  WITH CHECK (EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role = 'super_admin'));

DROP POLICY IF EXISTS "system_settings_delete" ON system_settings;
DROP POLICY IF EXISTS "system_settings_delete" ON system_settings; CREATE POLICY "system_settings_delete" ON system_settings FOR DELETE
  TO authenticated
  USING (EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role = 'super_admin'));
