    ...
    try {
      ...
      switch (reportType) {
        ...
        case 'collection': {
          let query = supabase
            .from('installments')
            .select('amount, status, due_date, collection_date, policy:policies!inner(agent_id, branch_id)')
          ...
          if (instError) {
            ...
            const collected = installments.filter(i => i.status === 'collected').reduce((sum, i) => sum + (i.amount as number), 0);
