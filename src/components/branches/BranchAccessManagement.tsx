...
export default function BranchAccessManagement() {
  ...
  async function handleAddAccess() {
    ...
    const { error } = await supabase
      .from('user_branch_access')
      .upsert(newAccess, { onConflict: 'user_id,branch_id' });
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-slate-800 rounded-2xl p-6 border border-slate-100 dark:border-slate-700 mb-6">
        ...
        <div className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">المستخدم</label>
            <select
              value={selectedUser || ''}
          ...
          <div>
            <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">الفروع</label>
              ...
              {branches.map(branch => (
                <label key={branch.id} className="flex items-center gap-3 p-2 hover:bg-slate-50 dark:hover:bg-slate-700/50 rounded-lg cursor-pointer">
                  <input
                    ...
                    onChange={(e) => {
                      if (e.target.checked) setSelectedBranches([...selectedBranches, branch.id]);
              ...
              {userAccess.length === 0 ? (
                ...
                <div className="flex flex-wrap gap-2">
                  {userAccess.map(access => {
                    ...
                    return branch ? (
                      <div key={access.id} className="flex items-center gap-2 px-3 py-1.5 bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400 rounded-lg text-sm">
                        <span>{branch.name}</span>
                        <button onClick={() => removeAccess(access.id)} className="text-blue-600 hover:text-red-600 dark:text-blue-300 dark:hover:text-red-400">
                          <X className="w-4 h-4" />
                        </button>
                      </div>
                    ) : null;
