-- ============================================================================
-- COMPLETE DATABASE BACKUP - Life Insurance Sales CRM
-- Date: 2026-06-25
-- ============================================================================

-- This backup contains:
-- 1. All table definitions (schema)
-- 2. All ENUMs
-- 3. All INDEXes
-- 4. All FUNCTIONS and TRIGGERS
-- 5. All VIEWS
-- 6. All RLS POLICIES
-- 7. All data (branches, users, hierarchy, clients, policies, collections, targets, settings)
-- 8. Auth users with hashed passwords
...
-- Auto-generate collections for new policies
  ...
  IF TG_OP = 'INSERT' AND NEW.status IN ('pending', 'active') THEN
    FOR i IN 1..12 LOOP
      ...
      INSERT INTO collections (
        policy_id, client_id, agent_id, branch_id, collection_number, amount, due_date, status
      ) VALUES (
        NEW.id, NEW.client_id, NEW.agent_id, NEW.branch_id, i, NEW.monthly_premium, due_date, 'pending'
      );
    END LOOP;
  END IF;
  RETURN NEW;
...
-- Update target when collection is paid
...
BEGIN
  IF TG_OP = 'UPDATE' AND NEW.status = 'paid' AND OLD.status != 'paid' THEN
    ...
    UPDATE targets SET achieved_amount = achieved_amount + NEW.amount, achieved_collections = achieved_collections + 1
  ...
  IF TG_OP = 'UPDATE' AND OLD.status = 'paid' AND NEW.status != 'paid' THEN
    ...
    UPDATE targets SET achieved_amount = GREATEST(0, achieved_amount - OLD.amount), achieved_collections = GREATEST(0, achieved_collections - 1)
...
-- Update target when policy is created
...
BEGIN
  IF TG_OP = 'INSERT' THEN
    ...
    UPDATE targets SET achieved_amount = achieved_amount + NEW.premium_amount, achieved_policies = achieved_policies + 1
    WHERE user_id = NEW.agent_id AND year = current_year AND month = current_month;
...
SELECT p.id, p.policy_number, p.policy_type, p.premium_amount, p.monthly_premium, p.issue_date, p.start_date, p.end_date, p.status,
  c.full_name AS client_name, c.phone AS client_phone, c.mobile AS client_mobile,
  a.full_name AS agent_name, a.email AS agent_email, b.name AS branch_name, b.code AS branch_code
FROM policies p
...
SELECT c.id, c.collection_number, c.amount, c.due_date, c.status,
  p.policy_number, p.policy_type, cl.full_name AS client_name, cl.phone AS client_phone, cl.mobile AS client_mobile,
...
SELECT t.id, t.user_id, p.full_name AS user_name, p.role AS user_role, b.name AS branch_name,
  t.year, t.month, t.target_amount, t.achieved_amount, t.target_policies, t.achieved_policies,
  t.target_collections, t.achieved_collections,
...
SELECT oh.id, oh.user_id, p.full_name AS user_name, p.email, p.role, b.name AS branch_name,
  oh.parent_id, pp.full_name AS parent_name, pp.role AS parent_role, oh.level
...
CREATE POLICY "profiles_select_super_admin" ON profiles FOR SELECT TO authenticated
  USING (EXISTS (SELECT 1 FROM profiles p WHERE p.id = auth.uid() AND p.role = 'super_admin') OR auth.uid() = id
...
CREATE POLICY "profiles_update_authenticated" ON profiles FOR UPDATE TO authenticated
  USING (auth.uid() = id OR EXISTS (SELECT 1 FROM profiles p WHERE p.id = auth.uid() AND p.role IN ('super_admin', 'development_manager'))
    OR EXISTS (SELECT 1 FROM get_user_descendants(auth.uid()) WHERE id = profiles.id))
  WITH CHECK (auth.uid() = id OR EXISTS (SELECT 1 FROM profiles p WHERE p.id = auth.uid() AND p.role IN ('super_admin', 'development_manager'))
    OR EXISTS (SELECT 1 FROM get_user_descendants(auth.uid()) WHERE id = profiles.id));
...
CREATE POLICY "clients_select_authenticated" ON clients FOR SELECT TO authenticated
  USING (EXISTS (SELECT 1 FROM profiles p WHERE p.id = auth.uid() AND p.role = 'super_admin') OR agent_id = auth.uid()
    OR EXISTS (SELECT 1 FROM get_user_descendants(auth.uid()) d WHERE d.id = clients.agent_id));
...
CREATE POLICY "policies_select_authenticated" ON policies FOR SELECT TO authenticated
  USING (EXISTS (SELECT 1 FROM profiles p WHERE p.id = auth.uid() AND p.role = 'super_admin') OR agent_id = auth.uid()
    OR EXISTS (SELECT 1 FROM get_user_descendants(auth.uid()) d WHERE d.id = policies.agent_id));
...
CREATE POLICY "collections_select_authenticated" ON collections FOR SELECT TO authenticated
  USING (EXISTS (SELECT 1 FROM profiles p WHERE p.id = auth.uid() AND p.role = 'super_admin') OR agent_id = auth.uid()
    OR EXISTS (SELECT 1 FROM get_user_descendants(auth.uid()) d WHERE d.id = collections.agent_id));
...
CREATE POLICY "targets_select_authenticated" ON targets FOR SELECT TO authenticated
  USING (EXISTS (SELECT 1 FROM profiles p WHERE p.id = auth.uid() AND p.role = 'super_admin') OR user_id = auth.uid()
    OR EXISTS (SELECT 1 FROM get_user_descendants(auth.uid()) d WHERE d.id = targets.user_id));
...
INSERT INTO branches (id, name, code, region, address, phone, email, manager_id, is_active) VALUES
  ('11111111-1111-1111-1111-111111111001', 'المركز الرئيسي', 'HQ001', 'الرياض', 'شارع الملك فهد، الرياض', '0112345678', 'main@insurance.com', '22222222-2222-2222-2222-222222222002', true),
  ('11111111-1111-1111-1111-111111111002', 'فرع الرياض', 'RYD001', 'الرياض', 'شارع العليا، الرياض', '0112345679', 'riyadh@insurance.com', '22222222-2222-2222-2222-222222222004', true),
  ('11111111-1111-1111-1111-111111111003', 'فرع جدة', 'JED001', 'جدة', 'شارع التحلية، جدة', '0129876543', 'jeddah@insurance.com', NULL, true)
ON CONFLICT (id) DO NOTHING;

-- ============================================================================
-- SEED DATA - AUTH USERS (Password: 123456)
-- ============================================================================

-- Note: In Supabase, auth.users are managed by Supabase Auth service.
-- For a complete restore, you would need to create these users through the Supabase dashboard
-- or use the Supabase Admin API. The password for all users is: 123456

-- ============================================================================
-- SEED DATA - PROFILES
-- ============================================================================

-- These must be created AFTER auth.users exist
-- User IDs: 22222222-2222-2222-2222-222222222001 through 010
...
-- ============================================================================
-- SEED DATA - DEFAULT TARGETS
-- ============================================================================

INSERT INTO targets (user_id, branch_id, year, month, target_type, target_amount)
SELECT '22222222-2222-2222-2222-222222222002', '11111111-1111-1111-1111-111111111001', EXTRACT(YEAR FROM now())::INTEGER, EXTRACT(MONTH FROM now())::INTEGER, 'premium', 750000
WHERE NOT EXISTS (SELECT 1 FROM targets WHERE user_id = '22222222-2222-2222-2222-222222222002' AND year = EXTRACT(YEAR FROM now())::INTEGER AND month = EXTRACT(MONTH FROM now())::INTEGER);

INSERT INTO targets (user_id, branch_id, year, month, target_type, target_amount)
SELECT '22222222-2222-2222-2222-222222222003', '11111111-1111-1111-1111-111111111001', EXTRACT(YEAR FROM now())::INTEGER, EXTRACT(MONTH FROM now())::INTEGER, 'premium', 240000
WHERE NOT EXISTS (SELECT 1 FROM targets WHERE user_id = '22222222-2222-2222-2222-222222222003' AND year = EXTRACT(YEAR FROM now())::INTEGER AND month = EXTRACT(MONTH FROM now())::INTEGER);

INSERT INTO targets (user_id, branch_id, year, month, target_type, target_amount)
SELECT '22222222-2222-2222-2222-222222222004', '11111111-1111-1111-1111-111111111002', EXTRACT(YEAR FROM now())::INTEGER, EXTRACT(MONTH FROM now())::INTEGER, 'premium', 240000
WHERE NOT EXISTS (SELECT 1 FROM targets WHERE user_id = '22222222-2222-2222-2222-222222222004' AND year = EXTRACT(YEAR FROM now())::INTEGER AND month = EXTRACT(MONTH FROM now())::INTEGER);
